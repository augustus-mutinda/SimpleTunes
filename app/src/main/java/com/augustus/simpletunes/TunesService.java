...
package com.augustus.simpletunes;
...
import com.augustus.simpletunes.helpers.MediaButtonIntentReceiver;
import com.augustus.simpletunes.helpers.MusicPlaybackTrack;
import com.augustus.simpletunes.lastfmapi.LastFmClient;
import com.augustus.simpletunes.lastfmapi.models.LastfmUserSession;
import com.augustus.simpletunes.lastfmapi.models.ScrobbleQuery;
import com.augustus.simpletunes.permissions.Nammu;
import com.augustus.simpletunes.provider.MusicPlaybackState;
import com.augustus.simpletunes.provider.RecentStore;
import com.augustus.simpletunes.provider.SongPlayCount;
...
import com.augustus.simpletunes.utils.TunesUtils.IdType;
...
@SuppressLint("NewApi")
public class TunesService extends Service {
    ...
    public void onCreate() {
        ...
        final Intent shutdownIntent = new Intent(this, TunesService.class);
    ...
    private void addToPlayList(final long[] list, int position, long sourceId, TunesUtils.IdType sourceType) {
    ...
    private void doAutoShuffleUpdate() {
        ...
        for (int i = 0; i < toAdd; i++) {
            ...
            mPlaylist.add(new MusicPlaybackTrack(mAutoShuffleList[idx], -1, TunesUtils.IdType.NA, -1));
            notify = true;
        }
        if (notify) {
            notifyChange(QUEUE_CHANGED);
        ...
        if (mRemoteControlClient != null) {
            ...
            if (what.equals(META_CHANGED) || what.equals(QUEUE_CHANGED)) {
                ...
                if (mShowAlbumArtOnLockscreen) {
                    albumArt = ImageLoader.getInstance().loadImageSync(TunesUtils.getAlbumArtUri(getAlbumId()).toString());
                    if (albumArt != null) {
    ...
    private Notification buildNotification() {
        ...
        Bitmap artwork;
        artwork = ImageLoader.getInstance().loadImageSync(TunesUtils.getAlbumArtUri(getAlbumId()).toString());
        ...
        if (artwork != null && TunesUtils.isLollipop()) {
            ...
            if (c != null && c.getCount() != 0) {
                ...
                do {
                    TrackItem t = new TrackItem()
                            .setArt(TunesUtils.getAlbumArtUri(c.getLong(c.getColumnIndexOrThrow(AudioColumns.ALBUM_ID))))
                            ...
                            .setDuration(TunesUtils.makeShortTimeString(this, c.getInt(c.getColumnIndexOrThrow(AudioColumns.DURATION)) / 1000));
                    list.add(t.get());
                } while (c.moveToNext());
                try {
    ...
    private final PendingIntent retrievePlaybackAction(final String action) {
        final ComponentName serviceName = new ComponentName(this, TunesService.class);
                ...
                try {
                    if (mCursor != null && shouldAddToPlaylist) {
                        ...
                        mPlaylist.add(new MusicPlaybackTrack(
                                mCursor.getLong(IDCOLIDX), -1, TunesUtils.IdType.NA, -1));
                        notifyChange(QUEUE_CHANGED);
                        mPlayPos = 0;
                        mHistory.clear();
                    }
    ...
    public void open(final long[] list, final int position, long sourceId, TunesUtils.IdType sourceType) {
        synchronized (this) {
            if (mShuffleMode == SHUFFLE_AUTO) {
...
//            }
    ...
    private static final class MusicPlayerHandler extends Handler {
        private final WeakReference<TunesService> mService;
        private float mCurrentVolume = 1.0f;

        public MusicPlayerHandler(final TunesService service, final Looper looper) {
            super(looper);
            mService = new WeakReference<TunesService>(service);
        }

        @Override
        public void handleMessage(final Message msg) {
            final TunesService service = mService.get();
            if (service == null) {
                return;
            }
    ...
    private static final class MultiPlayer implements MediaPlayer.OnErrorListener,
        ...
        public MultiPlayer(final TunesService service) {
            mService = new WeakReference<TunesService>(service);
    ...
    private static final class ServiceStub extends ISimpleTunesService.Stub {

        private final WeakReference<TunesService> mService;

        private ServiceStub(final TunesService service) {
            mService = new WeakReference<TunesService>(service);
        }

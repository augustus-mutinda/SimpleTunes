...
import com.augustus.simpletunes.dataloaders.AlbumLoader;
import com.augustus.simpletunes.dataloaders.AlbumSongLoader;
import com.augustus.simpletunes.dataloaders.ArtistAlbumLoader;
import com.augustus.simpletunes.dataloaders.ArtistLoader;
import com.augustus.simpletunes.dataloaders.ArtistSongLoader;
import com.augustus.simpletunes.dataloaders.PlaylistLoader;
import com.augustus.simpletunes.dataloaders.PlaylistSongLoader;
import com.augustus.simpletunes.dataloaders.SongLoader;
import com.augustus.simpletunes.models.Album;
import com.augustus.simpletunes.models.Artist;
import com.augustus.simpletunes.models.Playlist;
...
public class WearBrowserService extends MediaBrowserService {
    ...
    private final class MediaSessionCallback extends MediaSession.Callback {
        ...
        public void onPlayFromMediaId(final String mediaId, Bundle extras) {
            ...
            setSessionActive();
            TunesPlayer.playAll(mContext, new long[]{songId}, 0, -1, TunesUtils.IdType.NA, false);
        }
                    ...
                    switch (Integer.parseInt(Character.toString(parentId.charAt(0)))) {
                        case TYPE_ARTIST:
                            ...
                            for (Artist artist : artistList) {
                                String albumNmber = TunesUtils.makeLabel(mContext, R.plurals.Nalbums, artist.albumCount);
                                String songCount = TunesUtils.makeLabel(mContext, R.plurals.Nsongs, artist.songCount);
                                fillMediaItems(mediaItems, Integer.toString(TYPE_ARTIST_SONG_ALBUMS) + Long.toString(artist.id), artist.name, Uri.parse("android.resource://" +
                                        "naman14.timber/drawable/ic_empty_music2"), TunesUtils.makeCombinedString(mContext, albumNmber, songCount), MediaBrowser.MediaItem.FLAG_BROWSABLE);
                            }
                            break;
                        case TYPE_ALBUM:
                            List<Album> albumList = AlbumLoader.getAllAlbums(mContext);
                            for (Album album : albumList) {
                                fillMediaItems(mediaItems, Integer.toString(TYPE_ALBUM_SONGS) + Long.toString(album.id), album.title, TunesUtils.getAlbumArtUri(album.id), album.artistName, MediaBrowser.MediaItem.FLAG_BROWSABLE);
                            }
                            break;
                        case TYPE_SONG:
                            List<Song> songList = SongLoader.getAllSongs(mContext);
                            for (Song song : songList) {
                                fillMediaItems(mediaItems, String.valueOf(song.id), song.title, TunesUtils.getAlbumArtUri(song.albumId), song.artistName, MediaBrowser.MediaItem.FLAG_PLAYABLE);
                            }
                            break;
                        ...
                        case TYPE_ARTIST_SONG_ALBUMS:
                            ...
                            for (Album album : artistAlbums) {
                                String songCount = TunesUtils.makeLabel(mContext, R.plurals.Nsongs, album.songCount);
                                fillMediaItems(mediaItems, Integer.toString(TYPE_ALBUM_SONGS) + Long.toString(album.id), album.title, TunesUtils.getAlbumArtUri(album.id), songCount, MediaBrowser.MediaItem.FLAG_BROWSABLE);
                        ...
                        case TYPE_ARTIST_ALL_SONGS:
                            ...
                            for (Song song : artistSongs) {
                                fillMediaItems(mediaItems, String.valueOf(song.id), song.title, TunesUtils.getAlbumArtUri(song.albumId), song.albumName, MediaBrowser.MediaItem.FLAG_PLAYABLE);
                            }
                            break;
                        case TYPE_PLAYLIST:
                            List<Playlist> playlistList = PlaylistLoader.getPlaylists(mContext, false);
                            for (Playlist playlist : playlistList) {
                                String songCount = TunesUtils.makeLabel(mContext, R.plurals.Nsongs, playlist.songCount);

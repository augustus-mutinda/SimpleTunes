...
public class TunesPlayer {
    ...
    public static ISimpleTunesService mService = null;
    ...
    public static final ServiceToken bindToService(final Context context,
        ...
        contextWrapper.startService(new Intent(contextWrapper, TunesService.class));
        final ServiceBinder binder = new ServiceBinder(callback,
                contextWrapper.getApplicationContext());
        if (contextWrapper.bindService(
                new Intent().setClass(contextWrapper, TunesService.class), binder, 0)) {
    ...
    public static void asyncNext(final Context context) {
        final Intent previous = new Intent(context, TunesService.class);
        previous.setAction(TunesService.NEXT_ACTION);
    ...
    public static void previous(final Context context, final boolean force) {
        final Intent previous = new Intent(context, TunesService.class);
        if (force) {
            previous.setAction(TunesService.PREVIOUS_FORCE_ACTION);
        } else {
            previous.setAction(TunesService.PREVIOUS_ACTION);
        }
        context.startService(previous);
    }
        ...
        try {
            if (mService != null) {
                switch (mService.getRepeatMode()) {
                    case TunesService.REPEAT_NONE:
                        mService.setRepeatMode(TunesService.REPEAT_ALL);
                        break;
                    case TunesService.REPEAT_ALL:
                        mService.setRepeatMode(TunesService.REPEAT_CURRENT);
                        if (mService.getShuffleMode() != TunesService.SHUFFLE_NONE) {
                            mService.setShuffleMode(TunesService.SHUFFLE_NONE);
                        }
                        break;
                    default:
                        mService.setRepeatMode(TunesService.REPEAT_NONE);
                        break;
                }
            }
        } catch (final RemoteException ignored) {
        ...
        try {
            if (mService != null) {
                switch (mService.getShuffleMode()) {
                    case TunesService.SHUFFLE_NONE:
                        mService.setShuffleMode(TunesService.SHUFFLE_NORMAL);
                        if (mService.getRepeatMode() == TunesService.REPEAT_CURRENT) {
                            mService.setRepeatMode(TunesService.REPEAT_ALL);
                        }
                        break;
                    case TunesService.SHUFFLE_NORMAL:
                        mService.setShuffleMode(TunesService.SHUFFLE_NONE);
                        break;
                    case TunesService.SHUFFLE_AUTO:
                        mService.setShuffleMode(TunesService.SHUFFLE_NONE);
                        break;
                    default:
                        break;
    ...
    public static void playNext(Context context, final long[] list, final long sourceId, final IdType sourceType) {
        ...
        try {
            mService.enqueue(list, TunesService.NEXT, sourceId, sourceType.mId);
    ...
    public static void addToQueue(final Context context, final long[] list, long sourceId, IdType sourceType) {
        ...
        try {
            mService.enqueue(list, TunesService.LAST, sourceId, sourceType.mId);
            final String message = makeLabel(context, R.plurals.NNNtrackstoqueue, list.length);
            Toast.makeText(context, message, Toast.LENGTH_SHORT).show();
        } catch (final RemoteException ignored) {
        }

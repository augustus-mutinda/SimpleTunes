...
import com.augustus.simpletunes.adapters.SongsListAdapter;
import com.augustus.simpletunes.dataloaders.LastAddedLoader;
...
import com.augustus.simpletunes.dataloaders.TopTracksLoader;
...
import com.augustus.simpletunes.widgets.DragSortRecycler;
...
public class PlaylistDetailActivity extends BaseActivity implements ATEActivityThemeCustomizer, ATEToolbarCustomizer {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (animate && TunesUtils.isLollipop()) {
    ...
    private void setAlbumart() {
        ...
        loadBitmap(TunesUtils.getAlbumArtUri(getIntent().getExtras().getLong(Constants.ALBUM_ID)).toString());
    }
    ...
    private void clearAutoPlaylists() {
        switch (action) {
            case Constants.NAVIGATE_PLAYLIST_LASTADDED:
                TunesUtils.clearLastAdded(this);
                break;
            case Constants.NAVIGATE_PLAYLIST_RECENT:
                TunesUtils.clearRecent(this);
                break;
            case Constants.NAVIGATE_PLAYLIST_TOPTRACKS:
                TunesUtils.clearTopTracks(this);
                break;
        }
        Intent returnIntent = new Intent();
        setResult(Activity.RESULT_OK, returnIntent);

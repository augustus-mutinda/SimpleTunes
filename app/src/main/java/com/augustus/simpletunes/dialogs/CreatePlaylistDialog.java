package com.augustus.simpletunes.dialogs;
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        return new MaterialDialog.Builder(getActivity()).positiveText("Create").negativeText("Cancel").input("Enter playlist name", "", false, new MaterialDialog.InputCallback() {
            ...
            public void onInput(@NonNull MaterialDialog dialog, CharSequence input) {
                ...
                long playistId = TunesPlayer.createPlaylist(getActivity(), input.toString());

                if (playistId != -1) {
                    if (songs != null && songs.length != 0)
                        TunesPlayer.addToPlaylist(getActivity(), songs, playistId);
                    else

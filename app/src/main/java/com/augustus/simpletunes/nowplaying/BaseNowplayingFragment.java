...
import com.augustus.simpletunes.adapters.BaseQueueAdapter;
import com.augustus.simpletunes.adapters.SlidingQueueAdapter;
import com.augustus.simpletunes.dataloaders.QueueLoader;
import com.augustus.simpletunes.listeners.MusicStateListener;
import com.augustus.simpletunes.timely.TimelyView;
...
import com.augustus.simpletunes.widgets.CircularSeekBar;
import com.augustus.simpletunes.widgets.DividerItemDecoration;
import com.augustus.simpletunes.widgets.PlayPauseButton;
import com.augustus.simpletunes.widgets.PlayPauseDrawable;
...
public class BaseNowplayingFragment extends Fragment implements MusicStateListener {
        ...
        public void run() {
            ...
            if (mProgress != null) {
                ...
                if (elapsedtime != null && getActivity() != null)
                    elapsedtime.setText(TunesUtils.makeShortTimeString(getActivity(), position / 1000));
            }
            overflowcounter--;
    ...
    public Runnable mUpdateElapsedTime = new Runnable() {
        ...
        public void run() {
            if (getActivity() != null) {
                String time = TunesUtils.makeShortTimeString(getActivity(), TunesPlayer.position() / 1000);
                if (time.length() < 5) {
    ...
    private final View.OnClickListener mFLoatingButtonListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            duetoplaypause = true;
            if(TunesPlayer.getCurrentTrack() == null) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.menu_go_to_album:
                NavigationUtils.goToAlbum(getContext(), TunesPlayer.getCurrentAlbumId());
                break;
            case R.id.menu_go_to_artist:
                NavigationUtils.goToArtist(getContext(), TunesPlayer.getCurrentArtistId());
                break;
    ...
    public void setSongDetails(View view) {
        ...
        if (playPauseFloating != null) {
            playPauseDrawable.setColorFilter(TunesUtils.getBlackWhiteColor(accentColor), PorterDuff.Mode.MULTIPLY);
        ...
        if (mCircularProgress != null) {
            mCircularProgress.setOnSeekBarChangeListener(new CircularSeekBar.OnCircularSeekBarChangeListener() {
                ...
                public void onProgressChanged(CircularSeekBar circularSeekBar, int progress, boolean fromUser) {
                    if (fromUser) {
                        TunesPlayer.seek((long) progress);
                    }
                }
    ...
    public void updateSongDetails() {
        ...
        if (!duetoplaypause) {
            if (albumart != null) {
                ImageLoader.getInstance().displayImage(TunesUtils.getAlbumArtUri(TunesPlayer.getCurrentAlbumId()).toString(), albumart,
            ...
            if (songtitle != null && TunesPlayer.getTrackName() != null) {
                    songtitle.setText(TunesPlayer.getTrackName());
                    if(TunesPlayer.getTrackName().length() <= 23){
                        songtitle.setTextSize(25);
                    }
                    else if(TunesPlayer.getTrackName().length() >= 30){
                        songtitle.setTextSize(18);
                    }
                    else{
                        songtitle.setTextSize(18 + (TunesPlayer.getTrackName().length() - 24));
                    }
                    Log.v("BaseNowPlayingFrag", "Title Text Size: " + songtitle.getTextSize());
            }
            if (songartist != null) {
                songartist.setText(TunesPlayer.getArtistName());
            ...
            if (songalbum != null)
                songalbum.setText(TunesPlayer.getAlbumName());
        ...
        if (songduration != null && getActivity() != null)
            songduration.setText(TunesUtils.makeShortTimeString(getActivity(), TunesPlayer.duration() / 1000));
        ...
        if (mCircularProgress != null) {
            mCircularProgress.setMax((int) TunesPlayer.duration());
    ...
    public void notifyPlayingDrawableChange() {
        int position = TunesPlayer.getQueuePosition();
    ...
    private class loadQueueSongs extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String result) {
            if (result != null) {
                ...
                recyclerView.scrollToPosition(TunesPlayer.getQueuePosition() - 1);
            }

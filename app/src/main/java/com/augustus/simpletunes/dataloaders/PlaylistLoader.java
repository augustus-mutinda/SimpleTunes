...
package com.augustus.simpletunes.dataloaders;
...
public class PlaylistLoader {
    ...
    public static List<Playlist> getPlaylists(Context context, boolean defaultIncluded) {
        ...
        if (mCursor != null && mCursor.moveToFirst()) {
            do {
                ...
                final int songCount = TunesUtils.getSongCountForPlaylist(context, id);
    ...
    private static void makeDefaultPlaylists(Context context) {
        ...
        /* Last added list */
        final Playlist lastAdded = new Playlist(TunesUtils.PlaylistType.LastAdded.mId,
                resources.getString(TunesUtils.PlaylistType.LastAdded.mTitleId), -1);
        mPlaylistList.add(lastAdded);

        /* Recently Played */
        final Playlist recentlyPlayed = new Playlist(TunesUtils.PlaylistType.RecentlyPlayed.mId,
                resources.getString(TunesUtils.PlaylistType.RecentlyPlayed.mTitleId), -1);
        mPlaylistList.add(recentlyPlayed);

        /* Top Tracks */
        final Playlist topTracks = new Playlist(TunesUtils.PlaylistType.TopTracks.mId,
                resources.getString(TunesUtils.PlaylistType.TopTracks.mTitleId), -1);

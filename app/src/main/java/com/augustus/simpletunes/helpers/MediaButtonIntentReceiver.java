...
package com.augustus.simpletunes.helpers;
...
public class MediaButtonIntentReceiver extends WakefulBroadcastReceiver {
        ...
        public void handleMessage(final Message msg) {
            switch (msg.what) {
                ...
                case MSG_HEADSET_DOUBLE_CLICK_TIMEOUT:
                    ...
                    switch (clickCount) {
                        case 1:
                            command = TunesService.CMDTOGGLEPAUSE;
                            break;
                        case 2:
                            command = TunesService.CMDNEXT;
                            break;
                        case 3:
                            command = TunesService.CMDPREVIOUS;
                            break;
                        default:
                            command = null;
                            break;
    ...
    private static void startService(Context context, String command) {
        final Intent i = new Intent(context, TunesService.class);
        i.setAction(TunesService.SERVICECMD);
        i.putExtra(TunesService.CMDNAME, command);
        i.putExtra(TunesService.FROM_MEDIA_BUTTON, true);
    ...
    public void onReceive(final Context context, final Intent intent) {
        ...
        if (AudioManager.ACTION_AUDIO_BECOMING_NOISY.equals(intentAction)) {
            if (PreferencesUtility.getInstance(context).pauseEnabledOnDetach())
                startService(context, TunesService.CMDPAUSE);
            ...
            switch (keycode) {
                ...
                case KeyEvent.KEYCODE_MEDIA_PAUSE:
                    command = TunesService.CMDPAUSE;
                    break;
                case KeyEvent.KEYCODE_MEDIA_PLAY:
                    command = TunesService.CMDPLAY;
                    break;
            }
            if (command != null) {
                if (action == KeyEvent.ACTION_DOWN) {
                    if (mDown) {
                        if (TunesService.CMDTOGGLEPAUSE.equals(command)
                                || TunesService.CMDPLAY.equals(command)) {
                            if (mLastClickTime != 0

...
import com.augustus.simpletunes.widgets.BubbleTextGetter;
import com.augustus.simpletunes.widgets.MusicVisualizer;
...
public class SongsListAdapter extends BaseSongAdapter<SongsListAdapter.ItemHolder> implements BubbleTextGetter {
    ...
    public void onBindViewHolder(ItemHolder itemHolder, int i) {
        ...
        ImageLoader.getInstance().displayImage(TunesUtils.getAlbumArtUri(localItem.albumId).toString(),
                itemHolder.albumArt, new DisplayImageOptions.Builder().cacheInMemory(true)
                        .showImageOnLoading(R.drawable.ic_empty_music2)
                        .resetViewBeforeLoading(true).build());

        if (TunesPlayer.getCurrentAudioId() == localItem.id) {
                    ...
                    public boolean onMenuItemClick(MenuItem item) {
                        switch (item.getItemId()) {
                            case R.id.popup_song_remove_playlist:
                                TunesUtils.removeFromPlaylist(mContext, arraylist.get(position).id, playlistId);
                                removeSongAt(position);
                                notifyItemRemoved(position);
                                break;
                            case R.id.popup_song_play:
                                TunesPlayer.playAll(mContext, songIDs, position, -1, TunesUtils.IdType.NA, false);
                                break;
                            case R.id.popup_song_play_next:
                                long[] ids = new long[1];
                                ids[0] = arraylist.get(position).id;
                                TunesPlayer.playNext(mContext, ids, -1, TunesUtils.IdType.NA);
                                break;
                            ...
                            case R.id.popup_song_addto_queue:
                                ...
                                TunesPlayer.addToQueue(mContext, id, -1, TunesUtils.IdType.NA);
                                break;
                            case R.id.popup_song_addto_playlist:
                                AddPlaylistDialog.newInstance(arraylist.get(position)).show(mContext.getSupportFragmentManager(), "ADD_PLAYLIST");
                                break;
                            case R.id.popup_song_share:
                               TunesUtils.shareTrack(mContext, arraylist.get(position).id);
                                break;
                            case R.id.popup_song_delete:
                                long[] deleteIds = {arraylist.get(position).id};
                                TunesUtils.showDeleteDialog(mContext,arraylist.get(position).title, deleteIds, SongsListAdapter.this, position);
                                break;
                        }
                        return false;
                    }
